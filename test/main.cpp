...
class Callback : public relay::Callback {
  private:
    void on_new_message(std::set<relay::channel_id_t>,
                        bitstream &&data) override {
        int32_t pos = 0;
        data >> pos;
    ...
    void on_disconnect() override { DLOG(INFO) << "Got disconnect"; }
};
...
int main(int ac, char *av[]) {
    ...
    desc.add_options()("address", po::value<std::string>()->required(),
    ...
    Callback callback;
    auto conn = relay::create_connection(yael::network::resolve_URL(host, port),
                                         callback, {1, 3, 4, 7});
